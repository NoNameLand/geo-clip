...
# ----------------------------
# Geo encoder import (ensure local package is discoverable when running this script)
# ----------------------------
import sys
import os
...
def _find_project_root(start_path: str):
    ...
    while True:
        if os.path.exists(os.path.join(p, 'setup.py')) or os.path.isdir(os.path.join(p, 'geoclip_og')):
            return p
...
_proj_root = _find_project_root(__file__)
if _proj_root and _proj_root not in sys.path:
    sys.path.insert(0, _proj_root)

try:
    import geoclip_og  # must expose geoclip_og.LocationEncoder()
except Exception as e:
    raise ImportError(
        "Could not import 'geoclip_og'. Ensure you have installed the package in your environment or run this script from the project root.\n"
        f"Original error: {e}")
...
bundle_path = "experiments/alignment/models/geo_softprompt_model_cities.pt"  # from training script
...
init_image = Image.open("assets/sample_images/tel_aviv.jpg").convert("RGB").resize((512, 512))
