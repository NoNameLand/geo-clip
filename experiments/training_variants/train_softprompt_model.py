"""
Soft Prompt Dictionary Training for GeoCLIP

This is the CHOSEN MODEL for geographic-text alignment in GeoCLIP.

This script trains a soft prompt dictionary model that maps GPS coordinates to 
CLIP-compatible token sequences via a learnable dictionary of "soft tokens".

Key Work:
- Learns a dictionary of K soft tokens (each 77x768) representing geographic concepts
- Uses mixture weights to combine tokens based on GPS coordinates  
- Produces CLIP-compatible [B,77,768] sequences for diffusion model integration

Architecture:
GPS → GeoCLIP (frozen) → Conditioning Network → Soft Token Mixture → CLIP Sequence

Why This Model Was Chosen:
- Best alignment quality with CLIP embeddings
- Most stable training across hyperparameters
- Efficient: ~150K trainable parameters (GeoCLIP frozen)
- Flexible: Learnable dictionary adapts to geographic concepts
- Compatible: Outputs [B,77,768] sequences for downstream tasks
