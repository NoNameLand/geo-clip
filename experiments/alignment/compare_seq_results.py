...
# Ensure repository root is on sys.path so installed/editable packages and local imports resolve
# when running this script directly (avoids 'No module named geoclip_og').
...
if not os.path.exists(model_path):
    print(f"⚠️  Trained model not found at '{model_path}'. Skipping evaluation.\n"
          "Place the checkpoint at this path to run the full comparison.")
    # Exit gracefully; rest of script depends on model and data
    raise SystemExit(0)
else:
...
if not os.path.exists(test_path):
    print(f"⚠️  Test dataset not found at '{test_path}'. Skipping evaluation.\n"
          "Place the dataset CSV at this path to run the full comparison.")
    raise SystemExit(0)
import pandas as pd
