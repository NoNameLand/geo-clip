"""
GeoCLIP: A CLIP-inspired model for aligning images with geographical locations.

This module implements the main GeoCLIP model that combines an image encoder
with a location encoder to enable worldwide image geo-localization.
"""
...
class GeoCLIP(nn.Module):
    """
    GeoCLIP model for image geo-localization.
    
    This model combines a vision transformer for image encoding with a location
    encoder that uses random Fourier features to encode GPS coordinates. The model
    is trained contrastively to align images with their corresponding GPS locations.
    
    Args:
        from_pretrained (bool): Whether to load pre-trained weights. Default: True
        queue_size (int): Size of GPS queue for contrastive learning. Default: 4096
